	...
	% Hyperref
	\usepackage[
	pdfauthor={Laura Anger, Timo Breuer, Lukas Kolhagen},
...
\subsection{Bewegungsschätzung}\label{sec:bewegSch}
...
In Abbildung \ref{fig:beziehung} sieht man drei Bilder einer Bildsequenz. $Y_{n}$ steht hierbei für das $n$-te Bild dieser Sequenz und $y_{n}(k)$ ist der Grauwert des Pixels an der Stelle $k$. Das $k$ steht für ein Koordinatenpärchen ($k_{x},k_{y}$) und es gilt $k=1,\dots,N$, wobei $N$ die Anzahl der Pixel eines Bildes ist. Der eingezeichnete Vektor $v_{n}(k)$ ist derjenige Vektor, der auf $k$ addiert wird, sodass sich die Position des entsprechenden Pixels im vorherigen Bild ergibt. All diese Bewegungsvektoren zusammengenommen ergeben das Bewegungsvektorfeld eines Bildes, welches in der Abbildung mit $V_{n}$ bezeichnet wird. \\
...
Da es sich, wie bereits erwähnt, um einen bayesschen Ansatz handelt, gilt es sich mit bedingten Wahrscheinlichkeiten auseinander zu setzen. Der erste Faktor der Formel \ref{eq:bayes} ist im Originalansatz zur Modellierung des Rauschens gedacht. Es ist davon auszugehen, dass Röntgenbilder weit aus mehr verrauscht sind, als reale Bildsequenzen auf die das vorliegende Verfahren angewandt werden soll. Deshalb liegt es nahe, das Rauschen weitgehend zu vernachlässigen und durch einen reinen Datenterm zu ersetzen. Mehr Details dazu kann man in Kapitel \ref{sec:Datenterm} nachlesen. Die beiden anderen Faktoren können zum einen zur örtlichen und zum anderen zur zeitlichen Kohärenz vereinfacht werden. Diese Faktoren werden in den Kapiteln \ref{sec:oertlich} bzw. \ref{sec:zeitlich} näher beschrieben.
		
		\subsection{Kostenfunktion}\label{sec:costFunc}
			
Nach Aach und Kunz\textsuperscript{\cite{aach1998bayesian}} lässt sich die unter \ref{sec:Verfahren} erwähnte Wahrscheinlichkeitsdichte formulieren als:

\begin{equation}
ce^{-\displaystyle\sum\limits_{\vec{k}}(\frac{\left[d\left(\vec{k},\vec{v}\left(\vec{k}\right)\right)\right]^2}{4\cdot\sigma^2\left(\vec{k}\right)} + \lambda\sum_{\vec{l} \epsilon N(\vec{k})} g_{\vec{l}} \Vert \vec{v}(\vec{k}) - \vec{v}(\vec{l}) \Vert_2^{\nu} + \lambda_T\sum\limits_{\vec{k}}\left|\left|\hat{\vec{V}}_{n-1}\left(\vec{k}\right)-\vec{v}\left(\vec{k}\right)\right|\right|^\nu_2)}
\end{equation}

Der Ausdruck wird maximal, wenn der negative Exponent minimal wird.
Ziel der Aufgabe ist daher die Minimierung einer Kostenfunktion, deren Komponenten im folgenden Kapitel erläutert werden sollen. \\

Die Kostenfunktion lässt auf die wahrscheinlichste Bewegungsrichtung schätzen.
Zur gezielten Untersuchung werden 14 alternative Vektorkandidaten, die sich nach \cite{aach1998bayesian} ergeben, betrachtet.

Die Implementierung bestimmt auf Basis der aktuell untersuchten Bildposition die 14 alternativen Kandidaten für die Bewegungsschätzung. 
Die ermittelten Vektorkandidaten werden hierzu in einer Liste gehalten.
			...
			Um etwaige Probleme am Rand des Bildes zu verhindern, wurde eine Randbehandlung implementiert, welche verhindert, dass Pixeladressen im vorherigen ($y_{n-1}$) Bild außerhalb der Bildgrenzen liegen.
...
Hierzu werden neben dem neuen möglichen Bewegungsvektor der untersuchten Bildposition die Bewegungsvektoren der jeweiligen Achter-Nachbarschaft betrachtet.
...
\begin{equation}
p(V) = ce^{-\lambda E(V)} = c \prod_{\vec{k}}e^{-\lambda E_L(\vec{k},V)}
\end{equation}
...
Das Ergebnis wird für horizontale und vertikale Nachbarregionen mit dem Faktor $g_l=1$ und für diagonale Nachbarregionen mit dem Faktor $g_l=0.5$ gewichtet. Anschließend werden die gewichteten Ergebnisse aufsummiert und zurück gegeben.			
			...
			In (\ref{eq:zeitlich}) ist $\hat{\vec{V}}_{n-1}\left(\vec{k}\right)$ das geschätzte Bewegungsvektorfeld des vorherigen Bildes und $\vec{v}\left(\vec{k}\right)$ der Bewegungsvektor des aktuellen Blocks, welcher zur Position des Blocks im vorherigen Bildes führt. Auf $\lambda_T$ und $\nu$ wurde bereits im vorigen Abschnitt~\ref{sec:oertlich} eingegangen.
		...
		Die nächst äußere Schleife ist für die Iterationen zuständig. In den Iterationen wird innerhalb eines Bildes des Input-Stacks das Bewegungsvektorfeld mehrfach berechnet und jeweils das zuletzt berechnete Ergebnis als Grundlage für die nächste Berechnung verwendet. Dies ist erforderlich, um die Abweichung der Bewegungsvektoren pro Bild so gering wie möglich zu halten. Gleichzeitig hängt die Wahl der Anzahl der Iterationen natürlich davon ab, welche Performanz die Berechnung erreichen soll, also wie schnell ein Ergebnis benötigt wird.\\
		...
		%\begin{comment}
			...
			\centering
					\begin{psmatrix}[rowsep=0.3,colsep=0.4]
						\rnode{run}{\psframebox[fillstyle=solid,fillcolor=black]{\color{white}public void run(ImageProcessor ip)}}\\
						\rnode{readStack}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Read data from stack}}\\
						\rnode{initialize}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Initialize first motion vector field with zero vectors}}\\
						\rnode{forI}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{\texttt{$i=0$}}}\\
						\dianode[fillstyle=solid,fillcolor=red!30]{condStackSize}{$i<$ \texttt{stack size}?}\\[3mm]
						\dianode[fillstyle=solid,fillcolor=red!30]{frameNotZero}{$i\not=$ \texttt{0}?}\\[3mm]
						\rnode{curFrame}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Set current frame to i}}\\
						\rnode{forJ}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{\texttt{$j=0$}}}\\
						\dianode[fillstyle=solid,fillcolor=red!30]{condIter}{$j<$ \texttt{iterations}?}\\[3mm]
						\rnode{Vn}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Set current vector field as previous}}\\
						\rnode{forK}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{\texttt{$k=0$}}}\\
						\dianode[fillstyle=solid,fillcolor=red!30]{condNumBlocks}{$k<$ \texttt{number of blocks}?}\\[3mm]
						\rnode{calcAlt}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Calculate 14 alternatives for block k}}\\
						\rnode{minimizeCost}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Minimize cost function for alternatives}}\\
						\rnode{addVec}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Add vector with minimal cost to motion vector field}}\\
						\rnode{updateProgress}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Update plug-in progress}}\\
						\rnode{genMVF}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Generate motion vector field visualization for current frame}}\\
						\rnode{addToOutStack}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Add motion vector field to output stack}}\\
						\rnode{outWindow}{\psframebox[linearc=0.05,cornersize=absolute,fillstyle=solid,fillcolor=black!10]{Generate and display output window}}\\
					\end{psmatrix}
					...
					\ncangle[arm=2.8cm, angle=180, linearc=.1]{condStackSize}{outWindow}\nbput[npos=.2]{\color{red}False}
			\caption{Programmablauf}
			\label{fig:programmAblauf}
		\end{figure}
%\end{comment}		
			
\subsection{Visualisierung}\label{sec:vis}
	Um die Bewegungsvektorfelder $V_{n}$ darzustellen, wird sich mit Symbolen beholfen. Diese werden 				blockweise  über die Videosequenz gelegt. Aus Darstellungsgründen wird das Video dazu um Faktor 	4 in beide Bildrichtungen hoch skaliert. Da die einzelnen Bewegungsvektoren immer zwischen zwei Bildern berechnet werden, wird genau ein Bewegungsvektorfeld weniger erzeugt, als das 					Eingangsmaterial Bilder hat. \\
	...
	da hier durch den Algorithmus begründete Fehlberechnungen auftreten können.
	\section{Auswertung}

\subsection{Testmaterial}\label{sec:testmaterialien}
	Das Verfahren wird anhand von drei unterschiedlichen Bildsequenzen getestet, die im Folgenden 		Testmaterial 1--3 genannt werden und alle eine Auflösung von $256\times256$ Pixeln haben. Bei Testmaterial 1 und Testmaterial 2 wurde ein Bild um bekannte Werte in x- und y-Richtung	verschoben. Anzumerken ist auch, dass beide Testmaterialien identisch verschoben wurden. Bei Testmaterial 3 kann die Bewegung der zu sehenden Objekte lediglich geschätzt werden. Alle Testmaterialien wurden für diese Dokumentation aufgenommen bzw. erstellt.\\
...
\begin{minipage}{0.5\textwidth}
	...
	Diese Bildsequenz besteht aus 150 Bildern und wurde aus einem Standbild der Stadt Köln erzeugt.\\ 
	Bei den ersten und letzten 50 Bildern wird ein Ausschnitt des Standbildes um jeweils 10 Pixel 				nach links bzw. oben verschoben. In den mittleren 50 Bildern  wird der Bildinhalt um 10 				Pixel nach links und 10 Pixel nach unten verschoben. 
	\vspace{2.0cm}\\
\end{minipage}\\
...
\begin{minipage}{0.5\textwidth}
	...
	Auf den 150 Bildern, die diese Bildsequenz umfasst ist ein Blumenmotiv zu sehen, welches jeweils 	über 50 Bilder in verschiedene Richtungen verschoben wird. Auf den ersten Bildern erfolgt eine 		Verschiebung um 10 Pixel nach links. Während die mittleren Bilder um sowohl 10 Pixel nach 			links, als auch 10 Pixel nach oben verschoben wurden, wurde der Bildinhalt der verbleibenden 			Bilder nur um 10 Pixel nach oben verschoben.
	\vspace{0.9cm}\\
...
\begin{minipage}{0.5\textwidth}
	...
	Bei Testmaterial 3 handelt es sich um eine reale Videosequenz, die in Köln Ehrenfeld aufgenommen wurde. Zu sehen sind ein Auto, dass sich von links nach rechts durch das Bild bewegt und eine Fahrradfahrerin, die das Bild genau entgegengesetzt durchfährt. Beide Objekte werden zeitweise durch verschiedene Objekte verdeckt. Die Kamera ist starr, weshalb für den restlichen Bildinhalt keine Bewegung zu erkennen ist. Insgesamt besteht die Testsequenz aus 118 Bildern.
	\vspace{0.4cm}\\
\end{minipage}

\subsection{Auswertung der Testmaterialien}\label{sec:AuswertungTestmaterialien}
...
In Abbildung \ref{fig:testmaterial12} sind jeweils drei Bewegungsvektorfelder für Testmaterial 1 und Testmaterial 2 zu sehen. Es ist deutlich erkennbar, dass die Bewegungsdetektion sowohl für hochfrequente, als auch für tieffrequente Bildinhalte funktioniert hat.  Wie aus Kapitel \ref{sec:paraRegula} hervorgeht, ist davon auszugehen, dass mit kleineren Werten für $\lambda$ und $\lambda_{T}$ die Bewegungsvektorfelder schneller konvergieren.  \\
Die Abweichungen der berechneten von den tatsächlichen Bewegungsvektoren können bestimmt werden, da die Bewegung bekannt ist. Da sich alle Blöcke eines Bildes mit der selben Geschwindigkeit in die selbe Richtung bewegen, kann die Längenabweichung der einzelnen Vektoren über ein Bild aufsummiert und durch die Anzahl der Blöcke geteilt werden um ein Maß für die Abweichung zu erhalten.\\
Schaut man sich die so ermittelten Abweichungen für ein Bild an, wo in den vorherigen Bildern keine Richtungsänderung  stattgefunden hat, so beträgt diese im Durchschnitt für beide Testmaterialien ungefähr ein halbes Pixel. Da es wie in Kapitel \ref{sec:Einschwingverhalten} näher beschrieben dauert, bis die Bewegungsvektoren eingeschwungen sind, kommt es in den Bildern 50 und 100 in beiden Testmaterialien zu einer Abweichung von bis zu 8 Pixeln. Das sind eben die Bilder, wo sich die Bewegung plötzlich ändert. \\

\begin{figure}[H] 
		\centering
		\includegraphics[bb=0 0 1540 1024, width=\textwidth]{testmaterial12.jpg} 
			\caption{Bewegungsvektorfelder (v.\,l.\,n.\,r.): Bild 10, Bild 60 und Bild 110.\newline Oben Testmarial 1 und unten Testmaterial 2.}
...
Abbildung \ref{fig:frame43} und Abbildung \ref{fig:frame47} zeigen jeweils ein Standbild aus dem Bewegungsvektorfeld.\\
Es ist deutlich zu erkennen, dass die Bereiche in denen keine Bewegung stattfindet weitgehend richtig detektiert wurden. Und auch die Bewegungen des Autos und der Radfahrerin werden trotz der Verdeckung durch die Säule weitgehend erkannt. Auffällig ist, dass besonders die Blöcke, die hinter dem Auto liegen und deren Bildinhalt sich nicht mehr bewegt falsch berechnet sind.Hier befindet sich eine Objektgrenze, was vermutlich zu der Fehlberechnung geführt hat. Der Effekt ist besonders gut in Abbildung \ref{fig:frame47} zu sehen.\\

\begin{minipage}{0.5\textwidth}
	\begin{figure}[H] 
		\includegraphics[bb=0 0 517 516, width=0.9\textwidth]{frame43.png}
			\caption{Bewegungsvektorfeld \newline Testmaterial 3. Bild 43.}
		\label{fig:frame43}
	\end{figure}
\end{minipage}
\begin{minipage}{0.5\textwidth}
	\begin{figure}[H] 
		\includegraphics[bb=0 0 514 515, width=0.9\textwidth]{frame47.png}
			\caption{Bewegungsvektorfeld \newline Testmaterial 3. Bild 47.}
		\label{fig:frame47}
	\end{figure}
\end{minipage}\vspace{0.7cm}

Alle erzeugten Bewegungsvektorfelder können auf der beiliegenden Daten-CD eingesehen werden. Die Dateien heißen \textit{bvf-1.tif}, \textit{bvf-2.tif} und \textit{bvf-3.tif}.

\subsection{Einschwingverhalten} \label{sec:Einschwingverhalten}
Als Ausgangspunkt werden alle Bewegungsvektorfelder zu Beginn des Algorithmus auf den Nullvektor gesetzt. Das Verfahren muss sich also zunächst Einschwingen. Dabei wird im Folgenden zwischen dem interframe Einschwingen (über mehrere Bilder hinweg) und dem intraframe Einschwingen (innerhalb eines Bildes) unterschieden.
...
Neben den Einstellungen für $\nu,\lambda$ und $\lambda_T$ wird das Konvergenzverhalten durch den Bildinhalt des Testmaterials beeinflusst.
		...
		\subsubsection{Innerhalb eines Bildes}\label{sec:einschBild}
Wie in Kapitel \ref{sec:programmAblauf} beschrieben wird die Bewegungsdetektion in jedem Bild über mehrere Iterationen hinweg ermittelt. Mit anderen Worten: $V_{n}$ wird in jeder Iteration neu gesetzt. Im Folgenden wird die Veränderung der Vektoren nach jeder Iteration betrachtet. Dabei werden die Parameter wie in Kapitel \ref{sec:bvf} gewählt.\\
In Abbildung \ref{fig:einschwingIter5} werden die prozentualen Veränderungen der ersten sechs Bewegungsvektorfelder über 5 Iterationen hinweg aufgetragen. Es ist deutlich zu erkennen, dass bei den Bildern 1-4 nur eine kleine Veränderung der Bewegungsvektoren vorliegt. In Bild 5 geschieht die signifikante Änderung des Bewegungsvektorfeldes udn somit die eigentliche Einschwingung innerhalb des Bildes. In Bild 6 findet kaum noch eine Änderung statt. Dies entspricht Abbildung \ref{fig:einschwingFrame}, die das Einschwingverhalten über die einzelnen Bilder für 5 Iterationen zeigt. 
			...
			\begin{figure}[H]
				\psset{xunit=1cm,yunit=.5mm}
					...
					\listplot[linewidth=.025pt,plotstyle=dots]{
						...
						5	0.0}
			\end{pspicture}					
			\caption{Einschwingverhalten der Bewegungsvektorfelder für Testmaterial 1 in Abhängigkeit der Iterationen innerhalb eines Bildes bei fünf Iterationen.}
			\label{fig:einschwingIter5}
		\end{figure}
	%\end{comment}	
		
Während Abbildung \ref{fig:einschwingIter5} das bildweise Einschwingen für 5 Iterationen zeigt, wird in Abbildung \ref{fig:einschwingIter10} die prozentuale Abweichung der Bewegungsvektoren innerhalb eines Bildes für 10 Iterationen dargestellt.\\
Die Verdopplung der Iterationsschritte scheint dazu zu führen, dass bereits im dritten Bewegungsvektorfeld die Änderung gegen 0 geht und die Einschwingung bei einer gleichförmigen Bewegung wie in Testmaterial 1 zu sehen, beendet ist. 
			%\begin{comment}
			\begin{figure}[H]
				\psset{xunit=1cm,yunit=.5mm}
					\begin{pspicture}(-0.5,-20)(12,130)
					...
					\listplot[linewidth=.025pt,plotstyle=dots]{
						...
						10	00.385155}					
			\end{pspicture}					
			\caption{Einschwingverhalten der Bewegungsvektorfelder für Testmaterial 1 in Abhängigkeit der Iterationen innerhalb eines Bildes bei zehn Iterationen.}
			\label{fig:einschwingIter10}
		\end{figure}
	%\end{comment}		

		\subsection{Parameter der Regularisierungsterme}\label{sec:paraRegula}
...
In Tabelle \ref{tab:einschwingRegulaWerte} wird das Einschwingen der ersten 10 Bewegungsvektorfelder unter Variation von $\lambda$ und $\lambda_T$ betrachtet.
...
Die Wahl von $\lambda=0.001$ und $\lambda_T=0.001$ liefert das Optimum für die betrachteten Einstellungen.
	...
	\section{Zusammenfassung}\label{sec:upshot}
	...
	Die Implementierung wurde gemeinschaftlich und gleichverteilt bearbeitet. Die Aufteilung der Dokumentation gliedert sich wie folgt:
	\begin{description}
		\item[Anger:] Abschnitte \ref{sec:bewegSch}, \ref{sec:vis}, \ref{sec:testmaterialien}, \ref{sec:AuswertungTestmaterialien} und \ref{sec:einschBild}.
		\item[Breuer:] Abschnitte \ref{sec:costFunc}, \ref{sec:oertlich}, \ref{sec:bvf} und \ref{sec:paraRegula}
		\item[Kolhagen:] Abschnitte \ref{sec:einleitung}, \ref{sec:Datenterm}, \ref{sec:zeitlich}, \ref{sec:programmAblauf} und \ref{sec:upshot}.
	\end{description}
