...
\begin{figure}[H]
		\centering
		\includegraphics[scale=0.5]{beziehung-bild-bewegungsfeld.png}
		\caption{Beziehung zwischen Bewegungsvektorfeld und Einzelbildern [1]}
		\label{fig:beziehung}
	\end{figure}

In Abbildung \ref{fig:beziehung} sieht man drei Frames einer Bildsequenz. Y\textsubscript{n} steht hierbei für das n-te Bild dieser Sequenz und y\textsubscript{n}(k) ist der Grauwert des Pixels an der Stelle k. Das k steht für ein Koordinatenpärchen (k\textsubscript{x},k\textsubscript{y}) und es gilt k=1,...,N, wobei N die Anzahl der Pixel eines Frames ist. Der eingezeichnete Vektor v\textsubscript{n}(k) ist derjenige Vektor, der auf k addiert wird, sodass sich die Position des entsprechenden Pixels im vorherigen Bild ergibt. All diese Bewegungsvektoren zusammengenommen ergeben das Bewegungsvektorfeld eines Frames, welches in der Abbildung mit V\textsubscript{n} bezeichnet wird. \\
Das geschätzte Bewegungsvektorfeld \^{V}\textsubscript{n} ergibt sich nach den Ausführungen von Aach und Kunz zu: 
	
\begin{equation}
	\hat{V}\textsubscript{n}=arg\ \smash{\displaystyle\max_{V}} [p(Y\textsubscript{n},  Y\textsubscript{n-1},\vert V)\ \cdot \  p(\hat{V}\textsubscript{n-1} \vert V)\ \cdot \  p(V)]
\end{equation}\\

	
	
Da es sich um einen bayesschen Ansatz handelt, gilt es sich mit bedingten Wahrscheinlichkeiten auseinander zu setzen. Diese ergeben sich, 
	
	- verbessert sich 
	- Überleitung Kostenfunktion 
