...
Das geschätzte Bewegungsvektorfeld $\^{V}_{n}$ ergibt sich nach den Ausführungen von Aach und Kunz zu: 
	
\begin{equation}
	\hat{V}\textsubscript{n}=\arg\ \smash{\displaystyle\max_{V}} [p(Y_{n},  Y_{n-1},\vert V)\ \cdot \  p(\hat{V}_{n-1} \vert V)\ \cdot \  p(V)]
	\label{eq:bayes}
\end{equation}\\
